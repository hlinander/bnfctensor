...
freeIndices_ x s =
  case x
    ...
    Anon _ indices -> filter isFree indices
      where isFree index =
              not (indexLabelIn index s || occurences index indices > 1)
    ...
    Op _ indices expr -> filter isFree allIndices
      where isFree index =
              not (indexLabelIn index s || occurences index allIndices > 1)
            occurences x list = length $ filter (valenceFreeEq x) list
    ...
    Mul expr1 expr2 ->
      freeIndices_ expr1 (s ++ freeIndices_ expr2 s) ++
...
freeIndexSlots_ x s =
  case x
    ...
    Tensor _ indices -> zip (filter isFree indices) [0 ..]
      where isFree index =
              not
                (indexLabelIn index (map fst s) || occurences index indices > 1)
            occurences x list = length $ filter (valenceFreeEq x) list
    ...
    Op _ indices expr -> zip (filter isFree allIndices) [0 ..]
      where isFree index =
              not
                (indexLabelIn index (map fst s) ||
                 occurences index allIndices > 1)
            occurences x list = length $ filter (valenceFreeEq x) list
...
freeIndicesWithPos indices used = filter isFree (zip indices [0 ..])
  where
    isFree (index, _) =
      not (indexLabelIn index (map fst used) || occurences index indices > 1)
    occurences x list = length $ filter (valenceFreeEq x) list
