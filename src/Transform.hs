...
permData bs (Permute p c@(Tensor name _)) = (fromPermutation p, c, vinst)
  where
    vinst = lookupGeneratingSet $ fromJust $ M.lookup name $ bookTensors bs
...
simplify =
  fixPoint
    (commuteContractPermute .
     ...
     simplifyContract .
     eliminateMetrics .
     simplifyPermutations . simplifyTerms . simplifyFactors . sortCalc)
...
isSimilar (Prod (Number _) c1) c2
  | c1 == c2 = True
isSimilar c1 (Prod (Number _) c2)
  | c1 == c2 = True
isSimilar c1 c2
  | c1 == c2 = True
isSimilar _ _ = False
...
collectTerms' :: Calc -> Calc
collectTerms' c@(Sum s1 s2) =
  treeSum $
  (collectSumList .
   concat . (groupBy isSimilar) . (sortBy isSimilar') . flattenSum)
    c
collectTerms' x = x
...
prop_flattenTreeSum c =
  isCalcSum c ==> flattenSum c == (flattenSum $ treeSum $ flattenSum c)
...
simplifyTerms = transform sT
  where
    sT (Sum (Number n) (Number m)) = Number (n + m)
...
simplifyPermutations' :: Calc -> Calc
simplifyPermutations' (Prod (Permute p f1) f2) =
  Permute
    (concatPermutations p $ identity (length $ indexFromCalc f2))
    (Prod f1 f2)
simplifyPermutations' (Prod f1 (Permute p f2)) =
  Permute
    (concatPermutations (identity (length $ indexFromCalc f1)) p)
    (Prod f1 f2)
simplifyPermutations' (Permute p (Prod (Number n) f)) =
  Prod (Number n) (Permute p f)
simplifyPermutations' (Permute p (Sum t1 t2)) =
  Sum (Permute p t1) (Permute p t2)
simplifyPermutations' (Permute p (Permute q c)) = Permute (multiply p q) c
simplifyPermutations' (Permute p c)
  | isIdentityPermutation p = c
...
simplifyContract' :: Calc -> Calc
simplifyContract' (Contract i1 i2 (Sum t1 t2)) =
  Sum (Contract i1 i2 t1) (Contract i1 i2 t2)
simplifyContract' (Contract i1 i2 (Prod (Number n) f)) =
  Prod (Number n) (Contract i1 i2 f)
simplifyContract' (Prod (Contract i1 i2 c) f2) = Contract i1 i2 (Prod c f2)
simplifyContract' (Prod f1 (Contract i1 i2 c)) =
  Contract (i1 + n) (i2 + n) (Prod f1 c)
  where
    n = nFreeIndices f1
    ...
    f calc =
      case calc of
        Contract i1 i2 c
          ...
          | i1 < i2 ->
            case indexOnMetric c i1 of
              Nothing ->
                case indexOnMetric c i2 of
                  Nothing -> calc
                  Just (index, oim) -> eliminateOneMetric c i2 oim i1 index
              Just (index, oim) -> eliminateOneMetric c i1 oim i2 index
        x -> x

eliminateOneMetric c im oim it index =
  Permute perm $ removeMetric (replaceIndex c it index) im
  where
    perm = inverse $ sortingPermutationAsc posList
    [r1, r2] = sort [im, oim]
    posList =
      deleteAt r1 $ deleteAt r2 $ replaceAt it (oim) [0 .. (nFreeIndices c - 1)]
...
-- p(i1') = i1 => i1' = p^-1(i1)
commuteContractPermute' :: Calc -> Calc
commuteContractPermute' (Contract i1 i2 (Permute perm c)) =
  if permutationSize newPerm > 0
    then Permute newPerm (Contract i1'' i2'' c)
    else (Contract i1'' i2'' c)
  where
    list = [0 .. permutationSize perm]
...
validCalc x =
  case x of
    Permute p c ->
      (permutationSize p == length (indexFromCalc c)) && validCalc c
    Contract i1 i2 c ->
      inRange i1 0 (n - 1) &&
      inRange i2 0 (n - 1) &&
      (indexRepr (indices !! i1) == indexRepr (indices !! i2)) && validCalc c
...
leibnitz' :: Calc -> Calc
leibnitz' (Op l idx (Prod f1 f2)) =
  Prod (Op l idx f1) f2 |+| Prod f1 (Op l idx f2)
...
subCalc c pos = subCalc' c (map numberToInt pos)
  where
    numberToInt (Number n)
      | denominator n == 1 = fromIntegral $ numerator n
...
padGenSetLeft n (p, s) =
  concatPermutations (concatPermutations (identity n) p) s

padGenSetRight n (p, s) =
  concatPermutations (concatPermutations p (identity n)) s

prepGenSet gs = (p, s)
  where
    (l, sl) = splitAt (length gs - 2) gs
    p = toPermutation l
...
collectContractions' (Contract i1 i2 c) cs =
  collectContractions' c ([i1, i2] : cs)
collectContractions' c cs = (contractions, c)
  where
    popOne (ps, conts) [c1, c2]
      | c2 > c1 = (ps'', n1 : n2 : conts)
      where
        (ps', n2) = popAt c2 ps
        (ps'', n1) = popAt c1 ps'
    (_, contractions) =
      foldl (popOne) (emptyContractState $ nFreeIndices c) (cs)
...
reduceContractions c perm (d1:d2:rest) =
  reduceContractions (Contract i1 i2 c) perm' rest
  where
    i1 = unsafeElemIndex d1 perm
...
xPermToCalc c xperm dummies = Prod sign $ reduceContractions c lperm dummies
  where
    lperm = take (length xperm - 2) xperm
...
lowerIndices' bs t@(Tensor l idx) = calc
  where
    needFix = filter ((== Up) . indexValence . fst) (zip idx [0 ..])
