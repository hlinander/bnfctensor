...
xPerm ::
     [Int] -- ^ Permutation to canonicalise
  -> [[Int]] -- ^ Generating set
  -> [Int] -- ^ Base
  -> [Int] -- ^ Free indices
  -> [Int] -- ^ Dummy indices
  -> [Int]
xPerm perm gs base frees dummies =
  unsafePerformIO $ do
    result <-
      callXPerm
        (toCInt perm)
...
callXPerm ::
     [CInt] -- ^ Permutation to canonicalise
  -> [[CInt]] -- ^ Generating set
  -> [CInt] -- ^ Base
  -> [CInt] -- ^ Free indices
  -> [CInt] -- ^ Dummy indices
  -> IO [CInt]
callXPerm perm gs base frees dummies =
  allocaArray (length perm) $ \ptr ->
