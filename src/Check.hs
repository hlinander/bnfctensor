...
import Tensor (freeIndices, usedIndices)
...
analyzeStmt stmt =
  case stmt of
    StmtTensorDef labels tensordef ->
      analyzeTensorDef labels tensordef >> return stmt
    StmtOpDef labels opdef -> analyzeOpDef labels opdef >> return stmt
    StmtFuncDef ident exprs _ ->
      funcAppend (FunctionType (labelFromIdent ident) (length exprs)) >>
      return stmt
...
analyzeTensorDef lls def = do
  ...
  let labels = map labelsFromList lls
      ...
      maybeAppend l = do
        defined <- tensorDefined l
        if defined
          then fail "Tensor already defined: " -- ++ show $ lookupTensor l
          else tensorAppend l
  mapM_ (maybeAppend . tensor) labels

-- data Sym
--     = AntiSymmetric [LabelList]
--     | Symmetric [LabelList]
--     | Equality [LabelList] [EqSym]
--   deriving (Eq, Ord, Show, Read, Data, Typeable, Generic)
-- data EqSym = EqualityExpr SymExpr SymExpr
--   deriving (Eq, Ord, Show, Read, Data, Typeable, Generic)
-- data SymExpr = NegSym SymExpr | TensorSym Label [Index]
--   deriving (Eq, Ord, Show, Read, Data, Typeable, Generic)
...
analyzeOpDef lls def = do
  ...
  let labels = map labelsFromList lls
      ...
      maybeAppend l = do
        defined <- opDefined l
        if defined
          then fail "Op already defined: " -- ++ show $ lookupTensor l
          else opAppend l
...
analyzeExpr expr = do
  ...
  unless (nub indices == indices) $
    fail $ "Index label collision in [" ++ printTree expr ++ "]\n"
  do let exprMsg = "Index label collisions in [" ++ printTree expr ++ "]\n"
...
checkTensorDecl (Label s) indices = do
  ...
  case tensorType of
    ...
    Just (TensorType _ defIndices _)
      | length defIndices == length indices -> return ()
    Just (TensorType _ defIndices _) ->
      fail $
      "Tensor " ++ s ++ " used with wrong rank, expected " ++ (show defIndices)
...
findDeclTensor ::
     String -> ReaderT [Index] (StateT BookState Err) (Maybe TensorType)
findDeclTensor s = do
  bookState <- get
...
metricAppend r t =
  modify
    (\bs ->
       bs
         { bookTensors =
             case lookupTensor (tensorName t) bs of
...
tensorAppend tensor =
  modify
    (\t -> t {bookTensors = M.insert (tensorName tensor) tensor $ bookTensors t})
...
symmetryAppend sym = do
  ...
  let tt l = fromJust $ M.lookup l (bookTensors bs)
      ...
      makeAndInsertTypeOnTensor tensors l st =
        M.insert l (attachSymmetry st (tt l)) tensors
      updateTensors tensors anti lls =
        foldr
          (\l tensors ->
             makeAndInsertTypeOnTensor tensors l (symmetrize anti $ nIndices l))
          tensors
          lls
  case sym -- [1 2 3 4] -> [[1, 2], [2, 3], [3, 4], [4, 1]]
        of
    Symmetric lls ->
      modify
        (\bs ->
           bs
             { bookTensors =
                 updateTensors (bookTensors bs) False (unpackLabels lls)
             })
    AntiSymmetric lls ->
      modify
        (\bs ->
           bs
             { bookTensors =
                 updateTensors (bookTensors bs) True (unpackLabels lls)
             })
