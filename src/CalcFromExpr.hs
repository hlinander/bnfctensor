...
calcFromExpr' ::
     Abs.Expr -> (ReaderT BookState (Except String)) (Calc, [(String, Index)])
calcFromExpr' x =
  case x of
    ...
    Abs.Tensor (Abs.Label l) absIdx -> do
      ...
      tensorType <-
        asks (lookupTensor' ("Undefined tensor '" ++ l ++ "'") l) >>= liftEither
      bs <- ask
      let indexTypes = tensorIndices tensorType
      let freeSlots = T.freeIndexPos x
      let (indices, perm, contractions, sortedIdxAndLabels) =
            processIndexedObject absIdx indexTypes freeSlots
      calc <-
        case maybeCalc of
          Just storedCalc -> liftEither newCalc
            where storedIndices = indexFromCalc storedCalc
                  ...
                  needsFix =
                    filter
                      (\(v, rv, _) -> v /= rv)
                      (zip3 valences requestedValences [0 ..])
    ...
    Abs.Anon i absIdx -> do
      ...
      maybeCalc <-
        asks
          (lookupCalc'
             ("Undefined expression identifier '$" ++ show i ++ "'")
             varName) >>=
        liftEither
      tensorType <-
        asks
          (lookupTensor'
             ("Undefined tensor identifier '$" ++ show i ++ "'")
             varName) >>=
        liftEither
      let indexTypes = tensorIndices tensorType
      let freeSlots = T.freeIndexPos x
    ...
    Abs.Op (Abs.Label l) absIdx expr -> do
      (calc, idx) <- calcFromExpr' expr
      opType <-
        asks (lookupOp' ("Undefined operator '" ++ l ++ "'") l) >>= liftEither
      ...
      let (indices, perm, contractions, sortedIdxAndLabels) =
            processIndexedObject allIndices indexTypes freeSlots
      let contractedCalc =
            contractNew contractions (Op l (take (length absIdx) indices) calc)
      let permCalc =
            if isIdentityPermutation perm
              then contractedCalc
              else Permute perm contractedCalc
...
labeledIndexToAbsIndex (l, Index {indexValence = Down}) =
  Abs.Lower (Abs.Label l)
...
processIndexedObject ::
     [Abs.Index] -> [IndexType] -> [AbsIndexPos] -> IndexedData
processIndexedObject absIdx indexTypes freeSlots =
  (indices, perm, contractions, sortedIdxAndLabels)
  where
    absIndicesWithType = zip absIdx $ indexTypes :: [(Abs.Index, IndexType)]
    ...
    sortedIdxAndLabels =
      sortBy (\(a, _) (b, _) -> compare a b) freeIndicesAndLabels
...
processBinaryIndexed ::
     [LabeledIndex] -> [LabeledIndex] -> Int -> BinaryIndexedData
processBinaryIndexed idx1 idx2 offset = (perm, contracted, f')
  where
    contracted = contractedPairsNew idx1 idx2
...
contractNew (((_, _, i1), (_, _, i2)):rest) expr =
  Contract i1 i2 $ contractNew rest expr
...
contractedPairsNew ::
     [(String, Index)] -> [(String, Index)] -> [ContractPairNew]
contractedPairsNew free1 free2 = nestedPairs
          -- Get indices with same label between factors
  where
...
reduceNestedPairNew ::
     ContractPairNew -> ([Int], [ContractPairNew]) -> ([Int], [ContractPairNew])
reduceNestedPairNew ((l1, index1, i1), (l2, index2, i2)) (oldPos, newContractions)
  | i1 < i2 =
    (newPos, ((l1, index1, pos1), (l2, index2, pos2)) : newContractions)
  where
    pos1 = unsafeMaybe $ elemIndex i1 oldPos
...
selfContractedIndexPairs idxs = nestedPairs
  where
    indexSlotPositions = zip idxs [0 .. length idxs]
    indexFormat =
      map
        (\((absindex, index), i) -> (indexLabel absindex, index, i))
        indexSlotPositions
    distributedPairs =
      [ (i1, i2)
