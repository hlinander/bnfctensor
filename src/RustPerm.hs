...
foreign import ccall "canonicalize_free" callCFRust
  :: CLong -> Ptr CLong -> CLong -> Ptr CLong -> Ptr CLong -> IO ()
...
canonicalizeFree p gs =
  map ((+) 1) $
  map fromIntegral $
  unsafePerformIO $ canonicalizeFreeRust (toCLong p) (map toCLong gs)
