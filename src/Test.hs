...
instance Arbitrary Calc where
  ...
  arbitrary = sized foo
    where
      foo 0 =
        oneof [Number <$> arbitrary, arbitraryTensor, arbitrarySelfContraction]
      foo n =
        frequency
          [ (2, foo (n - 1))
          , (1, Number <$> arbitrary)
...
shrinkCalc c =
  case c of
    Prod f1 f2 ->
      [Tensor "shrinked" (indexFromCalc f1 ++ indexFromCalc f2)] ++
...
arbitraryCalc =
  oneof [Number <$> arbitrary, arbitraryTensor, arbitrarySum, arbitraryProduct]

isContractible :: Calc -> Bool
isContractible c =
  case map length indexGroups of
    [] -> False
    xs -> minimum xs >= 2
  where
    ...
    indexGroups =
      map snd $ M.toList $
      M.fromListWith (++) [(indexRepr i, [i]) | i <- indices]

prop_contract :: Int -> Int -> Int -> Property
prop_contract i1 i2 i = i1 >= 0 && i2 >= 0 && i >= 0 ==> 1 == 1

prop_replaceIndex :: Calc -> Int -> Index -> Property
prop_replaceIndex c i idx =
  length (indexFromCalc c) > i && i >= 0 ==>
  counterexample (renderConsole c ++ " -> " ++ renderConsole replaced) $
  (indexFromCalc replaced) !!
  i ==
  idx
  where
    replaced = replaceIndex c i idx

arbitraryContraction =
  suchThat arbitrary (\c -> length (indexFromCalc c) >= 2) >>= arbitraryContract
...
arbitraryIdentifier =
  vectorOf 2 (elements ['a' .. 'z']) >>= \(x:xs) -> return $ toUpper x : xs
...
prop_indexFromCalcUnderContraction c =
  length (indexFromCalc c) > 2 ==> forAll (arbitraryContract c) invariant
  where
    invariant c' = length (indexFromCalc c') == (length (indexFromCalc c)) - 2
...
prop_eliminateMetricsIsValid c =
  (length (indexFromCalc c) > 2) ==> forAll (arbitraryContract c) $ \calc -> do
...
prop_insertAt idx x list =
  idx >= 0 && idx <= length list ==> (U.insertAt idx x list) !! idx == x

prop_image :: Int -> Property
prop_image n = n >= 0 ==> forAll (shuffle [1 .. (n + 1) * 2]) assertion
  where
    assertion l =
      (permuteList (perm l) testList) !! (U.image (perm l) n) == testList !! n
    perm = toPermutation
    testList = [0 ..]
