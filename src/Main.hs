...
attemptCompletion ::
     BookState -> String -> Int -> Int -> IO (Maybe (String, [String]))
...
showTensor t =
  putStrLn $
  tensorName t ++
  " [" ++ (intercalate ", " (L.map show $ tensorIndices t)) ++ "]"
...
loadBook f =
  ...
  (\s ->
     L.map ((++ ";") . T.unpack) $
     L.filter (/= T.pack "") $ T.split (== ';') $ T.pack s) >>=
  (foldlM reduce emptyBook) >>=
  return . flip (,) ""
  where
    reduce bs s = do
      (bs', s') <- repl' bs s
